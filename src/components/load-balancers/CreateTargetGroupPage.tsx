...
import { lbApi, type ManagedTargetGroup, type CreateTgPayload } from "@/services/lbApi";
...
const REGION_VPC: Record<string, VpcItem> = {
...
export function CreateTargetGroupPage({ isAlb, vpcList, defaultVpcId, region, userId, onCancel, onCreate }: Props) {
  const TARGET_TYPE_TO_API: Record<TargetType, "instance" | "ip" | "lambda"> = {
    instances: "instance",
    ip: "ip",
    lambda: "lambda",
    alb: "instance",
  ...
  function validateHealthCheckPath(protocol: string, path: string): string | null {
  ...
  const vpcLabel = `${activeVpc.id} (${activeVpc.name})`;
                ...
                {showHealthCheckPath && (
                  <div className="space-y-3">
                    ...
                    <Input
                      ...
                      onChange={(e) => { setHealthCheckPath(e.target.value); setHealthCheckPathTouched(true); }}
                      onBlur={() => setHealthCheckPathTouched(true)}

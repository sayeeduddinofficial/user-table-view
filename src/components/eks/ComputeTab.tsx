    ...
    <div className="space-y-4">
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-sm font-semibold">
            Node pools <span className="text-muted-foreground font-normal">(0)</span>
          </h2>
        ...
        <p className="text-xs text-muted-foreground mb-3">
          Node pools define compute capacity for your Auto Mode cluster. Built-in
          node pools are managed by AWS, while custom node pools provide
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs uppercase tracking-wide text-muted-foreground border-b border-border">
                {["Name", "Type", "Status", "Node class", "Weight", "CPU limit", "Memory limit"].map((h) => (
                  <th key={h} className="px-4 py-2 text-left font-medium whitespace-nowrap">
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              <tr>
                <td colSpan={7} className="px-4 py-10 text-center text-muted-foreground">
                  ...
                  <div className="text-xs mt-1">
                    Add self managed node pools for use with Auto Mode using the Kubernetes API.
                  </div>
                </td>
              </tr>
            </tbody>
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        <h2 className="text-sm font-semibold mb-2">
          EKS node classes <span className="text-muted-foreground font-normal">(0)</span>
        </h2>
        <p className="text-xs text-muted-foreground mb-3">
            ...
            <tbody>
              <tr>
                <td colSpan={3} className="px-4 py-10 text-center text-muted-foreground">
                  ...
                  <div className="text-xs mt-1">
                    Add node classes for use with Auto Mode using the Kubernetes API.
                  </div>
                </td>
              </tr>
            </tbody>
      ...
      <div className="bg-card border border-border rounded-lg p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs uppercase tracking-wide text-muted-foreground border-b border-border">
                {["Group name", "Desired size", "AMI release version", "Launch template", "Status"].map((h) => (
                  <th key={h} className="px-4 py-2 text-left font-medium whitespace-nowrap">
                    {h}
                  </th>
                ))}
              ...
              {CLUSTER.nodeGroups.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">
                    <div className="font-medium text-foreground">No node groups</div>
                    <div className="text-xs mt-1">
                      This cluster does not have any node groups. When cluster creation is complete, you can add node groups.
                    </div>
                  </td>
                </tr>
              ) : (
                CLUSTER.nodeGroups.map((n) => (
                  <tr key={n.name} className="border-b border-border/40 last:border-0">
